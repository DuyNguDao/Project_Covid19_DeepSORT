...
def detect_5k(url_video=None, path_model=None, path_ds=None, flag_save=False, fps=None, name_video='video.avi'):
    """
    ...
    # load model DeepSORT
    dsoft_model = StrongSORT(model_weights=path_ds, device=device)
    ...
    while True:
        ...
        bbox, label, score, label_id = y5_model.predict(image)
        bboxs, labels, scores, labels_id = np.array(bbox), np.array(label), np.array(score), np.array(label_id)

        # *********************** GET ID PERSON AND AREA TRANSFORM ********************
        # count without mask
        count_without_mask = 0
        if len(bboxs) > 0:
            outputs = dsoft_model.update(bboxs, scores, labels_id, image)
            if len(outputs) > 0:
    ...
    cap.release()
    print('FPS average: {:d}'.format(int(total_frame / (time.time() - time_start))))
    if flag_save is True:
...
if __name__ == '__main__':
    ...
    path_ds = WEIGHTS / 'deep_sort/osnet_x0_25_msmt17.pth'
    # path video test
    ...
    detect_5k(url_video=url, path_model=path_models, path_ds=path_ds,
